...
from YeppoBot import CMD_HELP, DEVS, LOGS, STORAGE
...
@yeppo_cmd(pattern="clone ?(.*)", allow_sudo=False)
async def impostor(event):
    inputArgs = event.pattern_match.group(1)
    YeppoBot = ["@YeppoBot", "@YeppoBot"]
    if inputArgs in YeppoBot:
        await eor(event, get_string("yeppo_2"))
        await event.client.send_message("@YeppoBot", get_string("yeppo_3")
                                        )
        return
    ...
    if inputArgs:
        ...
        if replyMessage.sender_id in DEVS:
            return await xx.edit(get_string("yeppo_2")
                                 )
