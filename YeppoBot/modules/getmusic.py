...
from YeppoBot import CMD_HELP, LASTFM_USERNAME, bot, lastfm
from YeppoBot.yeppo import yeppo_cmd, chrome, progress
...
async def getmusicvideo(cat):
    ...
    for i in user_data:
        ...
        break
    command = f'youtube-dl -f "[filesize<50M]" --merge-output-format mp4 {video_link}'
    os.system(command)


@yeppo_cmd(pattern="songn (?:(now)|(.*) - (.*))")
async def _(event):
    if event.fwd_from:
        return
