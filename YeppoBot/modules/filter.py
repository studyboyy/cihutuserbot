...
from YeppoBot import BLACKLIST_CHAT, BOTLOG_CHATID
from YeppoBot import CMD_HANDLER as cmd
from YeppoBot import CMD_HELP, bot
from YeppoBot.events import yeppo_cmd, register
...
async def filter_incoming_handler(handler):
    ...
    try:
        if not (await handler.get_sender()).bot:
            try:
                from YeppoBot.modules.sql_helper.filter_sql import get_filters
...
@bot.on(yeppo_cmd(outgoing=True, pattern=r"filter (.*)"))
async def add_new_filter(new_handler):
    """For .filter command, allows adding new filters in a chat"""
    if new_handler.chat_id in BLACKLIST_CHAT:
        return await new_handler.edit(get_string("yeppo_1")
        )
    try:
        from YeppoBot.modules.sql_helper.filter_sql import add_filter
    except AttributeError:
...
@bot.on(yeppo_cmd(outgoing=True, pattern=r"stop (.*)"))
async def remove_a_filter(r_handler):
    """For .stop command, allows you to remove a filter from a chat."""
    try:
        from YeppoBot.modules.sql_helper.filter_sql import remove_filter
    except AttributeError:
...
@bot.on(yeppo_cmd(outgoing=True, pattern=r"delfilterbot (.*)"))
...
@bot.on(yeppo_cmd(outgoing=True, pattern=r"filters$"))
