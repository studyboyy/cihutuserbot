...
from YeppoBot.yeppo import yeppo_cmd, eor, time_formatter
...
@yeppo_cmd(pattern="sleep ([0-9]+)$", allow_sudo=False)
...
@yeppo_cmd(pattern="shutdown$", allow_sudo=False)
...
@yeppo_cmd(pattern="restart$", allow_sudo=False)
async def restart_bot(event):
    ...
    args = [sys.executable, "-m", "YeppoBot"]
    execle(sys.executable, *args, environ)


@yeppo_cmd(pattern="readme$")
...
@yeppo_cmd(pattern="repo$")
async def repo_is_here(event):
    yeppo = await eor(event, "🤖")
    sleep(3)
    await yeppo.edit(get_string("repo_1")
    )


@yeppo_cmd(pattern="raw$")
async def raw(event):
