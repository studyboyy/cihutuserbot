...
from YeppoBot.modules.yinsping import absen
from YeppoBot.yeppo import yeppo_cmd, get_user_from_event
from Stringyins import get_string


@yeppo_cmd(pattern="userid$")
...
@yeppo_cmd(pattern="inviteall ?(.*)")
async def get_users(event):
    yeppo_ = event.text[11:]
    chat_yeppo = yeppo_.lower()
    restricted = ["@YeppoBotSupport", "@YeppoBotsupport"]
    if chat_yeppo in restricted:
    ...
    yeppo = await eor(event, get_string("inal_5").format(yeppo_))
    yeppouserbot = await get_chatinfo(event)
    chat = await event.get_chat()
    if event.is_private:
        return await yeppo.edit(get_string("inal_4")
                                )
    s = 0
    f = 0
    error = "None"
    await yeppo.edit(get_string("inal_6"))
    async for user in event.client.iter_participants(yeppouserbot.full_chat.id):
        try:
            await event.client(InviteToChannelRequest(channel=chat, users=[user.id]))
            s += 1
            await yeppo.edit(get_string("inal_8").format(s, f, error)
                             )
        except Exception as e:
            error = str(e)
            f += 1
    return await yeppo.edit(get_string("inal_7").format(s, f)
                            )
...
CMD_HELP.update(
    {
        ...
        \n  •  **Contoh : **{cmd}link @YeppoBot Ganteng\
    "
    }
)
