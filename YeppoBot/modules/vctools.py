...
from YeppoBot.yeppo.pytgcalls import yeppo, CLIENTS, VIDEO_ON
...
@yeppo_cmd(pattern="startvc$", group_only=True)
...
@yeppo_cmd(pattern="stopvc$", group_only=True)
...
@yeppo_cmd(pattern="vcinvite", group_only=True)
async def _(c):
...
@yeppo_cmd(pattern="vctitle(?: |$)(.*)", group_only=True)
...
@yeppo_cmd(pattern="joinvc(?: |$)(.*)", group_only=True)
@register(incoming=True, from_users=GUA, pattern=r"^Joinvcs$")
async def _(event):
    sender = await event.get_sender()
    yins = await event.client.get_me()
    if sender.id != yins.id:
        YeppoBot = await event.reply(get_string("com_1"))
    else: 
        YeppoBot = await eor(event, get_string("com_1"))
    if len(event.text.split()) > 1:
        chat = event.text.split()[1]
        try:
            chat = await event.client.parse_id(chat)
        except Exception as e:
            return await eod(YeppoBot, get_string("error_1").format(str(e)))
    else:
        chat = event.chat_id
    Xd = yeppo(chat)
    if not Xd.group_call.is_connected:
        await Xd.group_call.join(chat)
        await YeppoBot.edit(get_string("jovc_1").format(yins.first_name, yins.id, chat)
        )
...
@yeppo_cmd(pattern="leavevc(?: |$)(.*)", group_only=True)
...
async def _(event):
    ...
    if len(event.text.split()) > 1:
        ...
        try:
            ...
            return await eod(yeppo, get_string("error_1").format(str(e)))
    else:
        chat = event.chat_id
    Xd = yeppo(chat)
    ...
    if VIDEO_ON.get(chat):
        del VIDEO_ON[chat]
    await YeppoBot.edit(get_string("levc_1").format(yins.first_name, yins.id, chat))


@yeppo_cmd(pattern="rejoin$")
