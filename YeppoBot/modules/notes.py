...
@yeppo_cmd(pattern="notes$")
async def notes_active(svd):
    try:
        from YeppoBot.modules.sql_helper.notes_sql import get_notes
...
@yeppo_cmd(pattern="clear (\\w*)")
async def remove_notes(clr):
    """For .clear command, clear note with the given name."""
    try:
        from YeppoBot.modules.sql_helper.notes_sql import rm_note
    except AttributeError:
...
@yeppo_cmd(pattern="save (\\w*)")
async def add_note(fltr):
    try:
        from YeppoBot.modules.sql_helper.notes_sql import add_note
    except AttributeError:
...
async def incom_note(getnt):
    ...
    try:
        if not (await getnt.get_sender()).getnt.client:
            try:
                from YeppoBot.modules.sql_helper.notes_sql import get_note
            except AttributeError:
                return
