...
from YeppoBot import BLACKLIST_CHAT, BOTLOG_CHATID, CLEAN_WELCOME
from YeppoBot import CMD_HANDLER as cmd
from YeppoBot import CMD_HELP, LOGS
from YeppoBot.yeppo import yeppo_cmd, chataction
...
async def welcome_to_chat(event):
    try:
        from YeppoBot.modules.sql_helper.welcome_sql import (
...
@yeppo_cmd(pattern="setwelcome(?: |$)(.*)")
async def save_welcome(event):
    if event.chat_id in BLACKLIST_CHAT:
        return await event.edit(get_string("yeppo_1"))
    try:
        from YeppoBot.modules.sql_helper.welcome_sql import add_welcome_setting
    except AttributeError:
...
@yeppo_cmd(pattern="checkwelcome$")
async def show_welcome(event):
    try:
        from YeppoBot.modules.sql_helper.welcome_sql import get_current_welcome_settings
    except AttributeError:
        return await event.edit(get_string("not_sql"))
...
@yeppo_cmd(pattern="rmwelcome$")
async def del_welcome(event):
    try:
        from YeppoBot.modules.sql_helper.welcome_sql import rm_welcome_setting
    except AttributeError:
        return await event.edit(get_string("not_sql"))
