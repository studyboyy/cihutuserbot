...
from YeppoBot import CMD_HELP, HEROKU_API_KEY, HEROKU_APP_NAME, SUDO_HANDLER, SUDO_USERS
...
@yeppo_cmd(pattern="addsudo(?:\\s|$)([\\s\\S]*)", allow_sudo=False)
async def add(event):
    suu = event.text[9:]
...
@yeppo_cmd(pattern="delsudo(?:\\s|$)([\\s\\S]*)", allow_sudo=False)
async def _(event):
    suu = event.text[8:]
