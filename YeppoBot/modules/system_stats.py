...
from YeppoBot import ALIVE_EMOJI, ALIVE_LOGO, ALIVE_TEKS_CUSTOM, BOT_VER, CHANNEL
from YeppoBot import CMD_HANDLER as cmd
from YeppoBot import CMD_HELP, GROUP, StartTime
from YeppoBot.yeppo import __version__ as py_ver
...
async def psu(event):
    ...
    help_string += f"**ᴋᴀᴢᴜ ᴠᴇʀsɪᴏɴ :** `{yeppo_version} [{HOSTED_ON}]`"
...
@yeppo_cmd(pattern="(?:alive|yinson)\\s?(.)?")
async def amireallyalive(alive):
    ...
    output = (
        ...
        f"{emoji} **ᴋᴀᴢᴜ ᴠᴇʀsɪᴏɴ :** `{yeppo_version}` [{HOSTED_ON}]\n"
