...
from YeppoBot import CMD_HELP, DEVS, WHITELIST
...
from YeppoBot.yeppo.logger import logging
...
@yeppo_cmd(pattern="setgpic( -s| -d)$", group_only=True)
...
@yeppo_cmd(pattern="promote(?:\\s|$)([\\s\\S]*)", group_only=True)
...
@yeppo_cmd(pattern="demote(?:\\s|$)([\\s\\S]*)", group_only=True)
...
@yeppo_cmd(pattern="ban(?:\\s|$)([\\s\\S]*)", group_only=True)
...
async def ban(bon):
    ...
    if not user:
        return
    yeppo = await eor(bon, get_string("band_1"))
    try:
        await bon.client(EditBannedRequest(bon.chat_id, user.id, BANNED_RIGHTS))
    except BadRequestError:
        return await eor(bon, get_string("no_perm"))
    await yeppo.edit(get_string("band_2").format(user.first_name, user.id, str(user.id), reason, me.first_name))


@yeppo_cmd(pattern="unban(?:\\s|$)([\\s\\S]*)", group_only=True)
...
@yeppo_cmd(pattern="mute(?: |$)(.*)", group_only=True)
@register(pattern=r"^\.cmute(?: |$)(.*)", sudo=True)
async def spider(spdr):
    try:
        from YeppoBot.modules.sql_helper.spam_mute_sql import mute
...
@yeppo_cmd(pattern="unmute(?: |$)(.*)", group_only=True)
...
async def unmoot(unmot):
    ...
    try:
        from YeppoBot.modules.sql_helper.spam_mute_sql import unmute
    except AttributeError:
...
async def muter(moot):
    try:
        from YeppoBot.modules.sql_helper.gmute_sql import is_gmuted
        from YeppoBot.modules.sql_helper.spam_mute_sql import is_muted
    except AttributeError:
        return
...
@yeppo_cmd(pattern="ungmute(?: |$)(.*)", group_only=True)
...
async def ungmoot(un_gmute):
    ...
    try:
        from YeppoBot.modules.sql_helper.gmute_sql import ungmute
    except AttributeError:
        return await eor(un_gmute, get_string("not_sql"), time=10)
    yeppo = await eor(un_gmute, get_string("com_1"))
    user = await get_user_from_event(un_gmute)
    user = user[0]
    if not user:
        return
    await yeppo.edit(get_string("ungm_1"))
    if ungmute(user.id) is False:
        await yeppo.edit(get_string("ungm_2"))
    else:
        await eor(un_gmute, get_string("ungm_3"), time=10)


@yeppo_cmd(pattern="gmute(?: |$)(.*)", group_only=True)
...
async def gspider(gspdr):
    ...
    try:
        from YeppoBot.modules.sql_helper.gmute_sql import gmute
    except AttributeError:
        return await gspdr.edit(get_string("not_sql"), time=10)
    yeppo = await eor(gspdr, get_string("com_1"))
    user, reason = await get_user_from_event(gspdr)
    if not user:
        return
    self_user = await gspdr.client.get_me()
    if user.id == self_user.id:
        return await yeppo.edit(get_string("mute_1"))
    if user.id in DEVS:
        return await yeppo.edit(get_string("gmut_1"))
    if user.id in WHITELIST:
        return await yeppo.edit(get_string("gmut_2"))
    await yeppo.edit(get_string("gmut_3"))
    if gmute(user.id) is False:
        await eor(gspdr, get_string("gmut_4"))
    elif reason:
        await yeppo.edit(get_string("gmut_5").format(user.first_name, user.id, user.id, reason, self_user.first_name)
                         )
    else:
        await yeppo.edit(get_string("gmut_6").format(user.first_name, user.id, user.id, self_user.first_name)
                         )


@yeppo_cmd(pattern="zombies(?: |$)(.*)", group_only=True)
...
@yeppo_cmd(pattern="admins$", group_only=True)
...
@yeppo_cmd(pattern="kick(?: |$)(.*)", group_only=True)
