...
@yeppo_cmd(pattern="exec(?: |$|\n)([\\s\\S]*)")
async def run(event):
    code = event.pattern_match.group(1)
    if not code:
        return await eor(event, get_string("devs_1"))
    if code in ("YeppoBot.session", "config.env"):
        return await eor(event, get_string("devs_2"))
