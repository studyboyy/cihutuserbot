...
from YeppoBot.yeppo import yeppo_cmd, edit_delete, edit_or_reply
...
@yeppo_cmd(pattern=r"font (monospace|smallcap|outline|bold|bolditalic|script|bubbles|blackbubbles) (.*)")
async def font_yins(yeppo):
    if yeppo.pattern_match.group(1) == "monospace":
        xd = monospace
    if yeppo.pattern_match.group(1) == "smallcap":
        xd = smallcap
    if yeppo.pattern_match.group(1) == "outline":
        xd = outline
    if yeppo.pattern_match.group(1) == "bold":
        xd = bold
    if yeppo.pattern_match.group(1) == "bolditalic":
        xd = bolditalic
    if yeppo.pattern_match.group(1) == "script":
        xd = script
    if yeppo.pattern_match.group(1) == "bubbles":
        xd = bubbles
    if yeppo.pattern_match.group(1) == "blackbubbles":
        xd = blackbubbles
    kontol = xd(yeppo.pattern_match.group(2))
    if not kontol:
        return await edit_delete(yeppo, "`Ngetik Yang Bener Bego...`")

    await yeppo.edit(f"{kontol}")
    await yeppo.reply("**𝙂𝙚𝙣𝙚𝙧𝙖𝙩𝙚𝙙 𝘽𝙮 :** ✧ 𝙺𝙰𝚉𝚄-𝚄𝚂𝙴𝚁𝙱𝙾𝚃 ✧")
...
@yeppo_cmd(pattern="font(.*)(|$)")
async def _(yeppo):
    input = yeppo.pattern_match.group(1).strip()
    reply = await yeppo.get_reply_message()
    reply_to_id = yeppo.message
    if yeppo.reply_to_msg_id:
        reply_to_id = await yeppo.get_reply_message()
    ...
    if not reply:
        try:
            ...
            return await edit_delete(yeppo, reply_to_id)
    elif not input:
        return await edit_delete(yeppo, "`Kasih Font Dong Bego :/`")

    else:
        font = input
        text = reply.message
    if not font:
        return await edit_delete(yeppo, f"`{font} Tidak Ada Dalam Daftar Font Kentod...`", time=5)
    if font == "smallcap":
    ...
    await edit_or_reply(yeppo, yins)
    await yeppo.reply("**𝙂𝙚𝙣𝙚𝙧𝙖𝙩𝙚𝙙 𝘽𝙮 :** ✧ 𝙺𝙰𝚉𝚄-𝚄𝚂𝙴𝚁𝙱𝙾𝚃 ✧")
    if not yins:
        await yeppo.reply("Ketik Yang Bener Bego!!!")
...
@yeppo_cmd(pattern="lf(.*)(|$)")
async def fonts(yins):
    yeppo = await edit_or_reply(yins,
                                "**»» ᴅᴀғᴛᴀʀ ғᴏɴᴛs ««**\n"
