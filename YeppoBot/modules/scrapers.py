...
from YeppoBot.modules.upload_download import get_video_thumb
...
from YeppoBot.yeppo.FastTelethon import upload_file
...
@yeppo_cmd(pattern="currency ([\\d\\.]+) ([a-zA-Z]+) ([a-zA-Z]+)")
...
@yeppo_cmd(pattern=r"lang (tr|tts) (.*)")
...
@yeppo_cmd(pattern="yt(audio|video( \\d{0,4})?) (.*)")
...
async def kbg(remob):
    ...
    if "image" in contentType:
        with io.BytesIO(output_file_name.content) as remove_bg_image:
            remove_bg_image.name = "yeppo_bg.png"
...
@yeppo_cmd(pattern=r"makeqr(?: |$)([\s\S]*)")
...
CMD_HELP.update(
    {
        ...
        \n  •  **Example :** `{cmd}google -l4 YeppoBot` atau `{cmd}google YeppoBot`\
    "
    }
)
