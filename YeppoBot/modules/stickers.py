...
from YeppoBot import S_PACK_NAME as custompack
from YeppoBot import tgbot
from YeppoBot.modules.sql_helper.globals import addgvar, gvarstatus
from YeppoBot.yeppo import yeppo_cmd, eod, eor
from YeppoBot.yeppo.misc import animator, create_quotly
...
@yeppo_cmd(pattern="(?:tikel|kang)\\s?(.)?")
async def kang(args):
...
@yeppo_cmd(pattern="pkang(?:\\s|$)([\\s\\S]*)")
async def _(event):
    xnxx = await eor(event, f"`{choice(KANGING_STR)}`")
    reply = await event.get_reply_message()
    query = event.text[7:]
    yeppoBot = await tgbot.get_me()
    BOT_USERNAME = yeppoBot.username
    bot_ = BOT_USERNAME
    bot_un = bot_.replace("@", "")
        ...
        try:
            create_st = await tgbot(
                functions.stickers.CreateStickerSetRequest(
                    user_id=OWNER_ID,
                    title=pname,
                    short_name=f"yeppo_{un_}_V{pack}_by_{bot_un}",
                    stickers=stcrs,
                )
            )
...
@yeppo_cmd(pattern="stickers ?([\\s\\S]*)")
