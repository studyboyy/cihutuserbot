...
from YeppoBot import CMD_HELP, BOTLOG_CHATID
...
@yeppo_cmd(pattern="btpm(?: |$)(.*)")
async def listbtpm(list):
    yeppo = await eor(list, get_string("com_1"))
    ...
    await yeppo.edit(f"**𝙱𝚃𝙿𝙼 𝙲𝙷:** {Brazzers.title}\n"
...
async def on_btpm(event):
    """ Fbtpm logic. """
    try:
        from YeppoBot.yeppo.btpm_yeppo import get_btpm
...
@yeppo_cmd(pattern=r"savebt (\w*)")
async def on_btpm_save(event):
    """ For .savebt command, saves btpm for future use. """
    try:
        from YeppoBot.yeppo.btpm_yeppo import add_btpm
...
@yeppo_cmd(pattern="listbt$")
async def on_btpm_list(event):
    """ For .listbt command, list btpm saved by you. """
    try:
        from YeppoBot.yeppo.btpm_yeppo import get_fbtpm
    except AttributeError:
        await event.edit(get_string("not_sql"))
        return
...
@yeppo_cmd(pattern=r"delbt (\w*)")
async def on_btpm_delete(event):
    """ For .delbt command, deletes a list btpm. """
    try:
        from YeppoBot.yeppo.btpm_yeppo import remove_btpm
    except AttributeError:
        await event.edit(get_string("not_sql"))
        return
