...
from YeppoBot import CMD_HELP, COUNT_PM, DEVS, LASTMSG, LOGS, PM_AUTO_BAN, PM_LIMIT, bot
from YeppoBot.events import yeppo_cmd
from YeppoBot.yeppo import eod, eor
...
async def permitpm(event):
    ...
    if (
        ...
        try:
            ...
            from YeppoBot.modules.sql_helper.pm_permit_sql import is_approved
...
async def auto_accept(event):
    ...
    if (
        ...
        try:
            from YeppoBot.modules.sql_helper.globals import gvarstatus
            from YeppoBot.modules.sql_helper.pm_permit_sql import approve, is_approved
        except AttributeError:
            return
...
@bot.on(yeppo_cmd(outgoing=True, pattern=r"notifoff$"))
async def notifoff(noff_event):
    """For .notifoff command, stop getting notifications from unapproved PMs."""
    try:
        from YeppoBot.modules.sql_helper.globals import addgvar
    except AttributeError:
...
@bot.on(yeppo_cmd(outgoing=True, pattern=r"notifon$"))
async def notifon(non_event):
    """For .notifoff command, get notifications from unapproved PMs."""
    try:
        from YeppoBot.modules.sql_helper.globals import delgvar
    except AttributeError:
...
@bot.on(yeppo_cmd(outgoing=True, pattern=r"(?:setuju|ok)\s?(.)?"))
async def approvepm(apprvpm):
    """For .ok command, give someone the permissions to PM you."""
    try:
        from YeppoBot.modules.sql_helper.globals import gvarstatus
        from YeppoBot.modules.sql_helper.pm_permit_sql import approve
    except AttributeError:
...
@bot.on(yeppo_cmd(outgoing=True, pattern=r"(?:tolak|nopm)\s?(.)?"))
async def disapprovepm(disapprvpm):
    try:
        from YeppoBot.modules.sql_helper.pm_permit_sql import dissprove
    except BaseException:
...
@bot.on(yeppo_cmd(outgoing=True, pattern=r"block$"))
...
@bot.on(yeppo_cmd(outgoing=True, pattern=r"unblock$"))
...
@bot.on(yeppo_cmd(outgoing=True, pattern=r"(set|get|reset) pmpermit(?: |$)(\w*)"))
async def add_pmsg(cust_msg):
    ...
    try:
        import YeppoBot.modules.sql_helper.globals as sql
    except AttributeError:
...
async def pmdevs(event):
    ...
    try:
        from YeppoBot.modules.sql_helper.globals import gvarstatus
        from YeppoBot.modules.sql_helper import pm_permit_sql as yins_sql
    except AttributeError:
    ...
    if event.is_private:
        ...
        if not yins_sql.is_approved(devs.id):
            try:
                ...
                await bot.send_message(BOTLOG_CHATID, f"**#AUTO_APPROVED_DEVELOPER**\n\n👑 **Developer:** [{devs.first_name}](tg://user?id={devs.id})\n💬 `Developer Ichihood-Userbot Telah Mengirimi Anda Pesan...`")
                await bot.send_message(
                    devs, f"**Menerima Pesan!!!**\n**Terdeteksi [{devs.first_name}](tg://user?id={devs.id}) Adalah Developer Ichihood-Userbot**"
                )
