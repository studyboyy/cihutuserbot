...
from YeppoBot import DB_URI, LOGS
...
class MongoDB:
    def __init__(self, key, dbname="yeppoDB"):
        self.dB = MongoClient(key, serverSelectionTimeoutMS=5000)
        self.db = self.dB[dbname]
        self.re_cache()

    def __repr__(self):
        return f"<yeppo.MonGoDB\n -total_keys: {len(self.keys())}\n>"
...
class SqlDB:
    def __init__(self, url):
        ...
        try:
            ...
            self._cursor.execute(
                "CREATE TABLE IF NOT EXISTS yeppo (yeppoCli varchar(70))"
            )
    ...
    def usage(self):
        self._cursor.execute(
            "SELECT pg_size_pretty(pg_relation_size('yeppo')) AS size"
        )
    ...
    def keys(self):
        self._cursor.execute(
            "SELECT column_name FROM information_schema.columns WHERE table_schema = 'public' AND table_name  = 'yeppo'"
        )  # case sensitive
        data = self._cursor.fetchall()
    ...
    def get(self, variable):
        try:
            self._cursor.execute(f"SELECT {variable} FROM yeppo")
    ...
    def set_key(self, key, value):
        try:
            self._cursor.execute(f"ALTER TABLE yeppo DROP COLUMN IF EXISTS {key}")
        except (psycopg2.errors.UndefinedColumn, psycopg2.errors.SyntaxError):
            pass
        except BaseException as er:
            LOGS.exception(er)
        self._cache.update({key: value})
        self._cursor.execute(f"ALTER TABLE yeppo ADD {key} TEXT")
        self._cursor.execute(f"INSERT INTO yeppo ({key}) values (%s)", (str(value),))
        return True
    ...
    def del_key(self, key):
        if key in self._cache:
            del self._cache[key]
        try:
            self._cursor.execute(f"ALTER TABLE yeppo DROP COLUMN {key}")
        except psycopg2.errors.UndefinedColumn:
            return False
        return True
    ...
    def flushall(self):
        self._cache.clear()
        self._cursor.execute("DROP TABLE yeppo")
        self._cursor.execute(
            "CREATE TABLE IF NOT EXISTS yeppo (yeppoCli varchar(70))"
        )
        return True
